                                ...
                                {action.resolutionType === 'MULTI_HIT' && action.hitResolutions.map(res => (
                                    <div key={`${res.hitNumber}-${res.target.id}`} className="grid grid-cols-[1fr_auto] items-center gap-x-4 p-2 bg-gray-800/50 rounded">
                                        ...
                                        <div className="flex items-center gap-x-4">
                                            {action.canMegaEvolve && (
                                                <label className="flex items-center gap-2 cursor-pointer text-sm font-bold text-purple-400 hover:text-white">
                                                    <input
                                                        type="checkbox"
                                                        checked={!!dmOverrides[`willMegaEvolve_${action.attacker.id}`]}
                                                        onChange={(e) => handleOverrideChange(`willMegaEvolve_${action.attacker.id}`, e.target.checked)}
                                                        className="form-checkbox h-4 w-4 bg-gray-700 border-gray-500 rounded text-purple-500 focus:ring-purple-600"
                                                    />
                                                    Mega Evolve?
                                                </label>
                                            )}
